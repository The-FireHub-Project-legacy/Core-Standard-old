...
use FireHub\Core\Support\Exceptions\FileSystem\ {
    CopyPathException, CreateLinkException, DeletePathException, GetContentAsArrayException, GetContentException,
...
final class File extends FileSystem {
    ...
    public static function size (string $path):int {
        ...
        return ($size = filesize($path)) !== false
            ? $size : throw new GetPathSizeException()->fromPath($path);
    ...
    public static function delete (string $path):true {
        ...
        return unlink($path)
            ?: throw new DeletePathException()->fromPath($path);
    ...
    public static function putContent (string $path, array|string $data, bool $append = false, bool $lock = true, bool $create_file = false):int {
        ...
        if (!$create_file && !self::isFile($path)) throw new PathDoesntExistException()->fromPath($path);
    ...
    public static function read (string $path):int {
        ...
        return readfile($path)
            ?: throw new ReadFileException()->fromPath($path);
