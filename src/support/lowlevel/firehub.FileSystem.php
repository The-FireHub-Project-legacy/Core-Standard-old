...
class FileSystem {
    ...
    final public static function absolutePath (string $path):string {
        ...
        return realpath($path)
            ?: throw new GetAbsolutePathException()->fromPath($path);
    ...
    final public static function getGroup (string $path):int {
        ...
        return filegroup($path)
            ?: throw new GetGroupException()->fromPath($path);
    ...
    final public static function getOwner (string $path):int {
        ...
        return fileowner($path)
            ?: throw new GetOwnerException()->fromPath($path);
     ...
     * </p>
     *
     * @throws \FireHub\Core\Support\Exceptions\FileSystem\GetPermissionsException If we couldn't get permissions for
     * a path.
     *
    ...
    final public static function getPermissions (string $path):string {
        ...
        return DataIs::int($permissions = fileperms($path))
            ...
            : throw new GetPermissionsException()->fromPath($path);
    ...
    final public static function lastAccessed (string $path):int {
        ...
        return ($time = fileatime($path)) !== false
            ? $time : throw new GetLastAccessedException()->fromPath($path);
    ...
    final public static function lastModified (string $path):int {
        ...
        return ($time = filemtime($path)) !== false
            ? $time : throw new GetLastModifiedException()->fromPath($path);
    ...
    final public static function lastChanged (string $path):int {
        ...
        return ($time = filectime($path)) !== false
            ? $time : throw new GetLastChangedException()->fromPath($path);
    ...
    final public static function inode (string $path):int {
        ...
        return fileinode($path)
            ?: throw new GetInodeException()->fromPath($path);
    ...
    final public static function getSymlink (string $path):string {
        ...
        return readlink($path)
            ?: throw new GetSymlinkException()->fromPath($path);
     ...
     * a path.
     *
     * @return array{dev: int, ino: int, mode: int, nlink: int, uid: int, gid: int, rdev: int, size: int, atime: int,
     *      mtime: int, ctime: int, blksize: int, blocks: int} Statistics about a file or folder.
     */
