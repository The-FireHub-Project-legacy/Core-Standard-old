        ...
        foreach ($this->bootstraps as $key => $value)
            match (true) {
                DataIs::string($key) && DataIs::array($value) && Cls::ofClass($key, Bootstrap::class)
                    => new $key(...$value)->load() ?: throw new FailedToLoadBootstrapException()->fromClass($key),
                (DataIs::string($value) || DataIs::object($value)) && Cls::ofClass($value, Bootstrap::class)
                    => new $value()->load() ?: throw new FailedToLoadBootstrapException()->fromClass($value),
                default => throw new NotBootstrapException()->fromClass($value)
            };
